...
/////////////////////////
  static getUnVarifiedReceipts = async (req, res, next) => {
    ...
    try {
        const installment = await InstallmentReceipts.findAll({ include: [{ as: 'Member', model: Member },{ as: 'Payment_Mode', model: Payment_Mode }]
            , where: {
                [Op.or]: [
                    { AdminVarified: null },
                    { AdminVarified: 0 }
...
static getUnVarifiedPdfByUsers = async (req, res, next) => {
  try {
    // const checkedIds  = req.query.id.split(',');
    const { checkedIds } = req.body;
    const installment = await InstallmentReceipts.findAll({ include: [{ as: 'Member', model: Member }, { as: 'Payment_Mode', model: Payment_Mode }, { as: 'User', model: User }, { as: 'Booking', model: Booking }] 
    , where: {
      INS_RC_ID: checkedIds,
        [Op.or]: [
          { AdminVarified: null },
    ...
    const groupedData = installment.reduce((acc, item) => {
      ...
      acc[USER_ID].items.push({ ...rest, payOrderTotal: +item.Installment_Due + acc[USER_ID].payOrderTotal, total: +item.Installment_Paid + acc[USER_ID].total }); // Push the item with updated totalAmount and totalQuantity
...
//////////////////////////////////////////////
  ...
  static update = async (req, res, next) => {
    ...
    try {
      ...
      result = await InstallmentReceipts.update({ ...req.body, LAST_UPDATE: Date.now() }, { where: { INS_RC_ID: installmentId }, returning: true })
      res.status(200).json({
        "status": 200,
