  ...
  static addfileTransfer = async (req, res, next) => {
    ...
    try {
      ...
      if (TRSR_ID) {
        ...
        const allBookingDetails = await BookingInstallmentDetails.findAll(
          { where: { BK_ID: BK_ID } }
        );


        const allBookingInstallmentReceipts = await InstallmentReceipts.findAll(
          { where: { BK_ID: BK_ID } }
        );


        for(let k = 0; k < allBookingDetails.length; k++) {
          const bInstallments = allBookingInstallmentReceipts.filter((it) => it.BKI_DETAIL_ID == allBookingDetails[k].BKI_DETAIL_ID);
          ...
          if(bInstallments.length > 0) {
            ...
            if(remaining > 0) {
              await BookingInstallmentDetails.update(
                { MEMBER_ID: BUYER_MEMBER_ID },
                { where: { 
                    BK_ID: BK_ID, BKI_DETAIL_ID: allBookingDetails[k].BKI_DETAIL_ID }
                }
              );
            }
          } else {
