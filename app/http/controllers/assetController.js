...
class AssetController {
    static addAsset = async (req, res, next) => {
        ...
        if (
            name && type && model && description && expiryDate && quantity && addedDate
        ) {
    ...
    static getAssetByEmployeeId = async (req, res, next) => {
        ...
        try {
        const asetById = await employeeAsset.findAll({ include: [{ as: 'AssetType', model:AssetType }], where: { employeeId: employeeId } })
        if (asetById.length > 0) {
            res.status(200).send({
                status: 200 ,
                "message": "get Asset successfully",
    ...
    static getAllAsset = async (req, res) => {
        const allAsset = await Asset.findAll({order: [['createdAt', 'DESC']],include: [{ as: 'AssetType', model:AssetType }]});
        console.log('called');
        if (allAsset !== null) {
            res.status(200).send({
                status: 200 ,
