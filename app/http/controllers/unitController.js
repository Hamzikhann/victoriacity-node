...
class UnitController {
    ...
    static getUnitById = async (req, res, next) => {
        ...
        try {
            const unit = await Unit.findOne({ include: [{ as: 'Block', model: Block }, { as: 'UnitType', model: UnitType }, { as: 'PlotSize', model: PlotSize }, { as: 'Member', model: Member }, { as: 'MemNominee', model: MemNominee }, { as: 'Street', model: Street }], where: { ID: UnitId } })
            if (!unit) {
    ...
    static getAllUnit = async (req, res, next) => {
        ...
        try {
            const { count, rows } = await Unit.findAndCountAll({
                include: [{ as: 'Block', model: Block }, { as: 'UnitType', model: UnitType }, { as: 'PlotSize', model: PlotSize }, { as: 'Member', model: Member }, { as: 'MemNominee', model: MemNominee }, { as: 'Street', model: Street }], where: { IsDeleted: 0 },
                offset: limit * page,
                limit: limit,
    ...
    static unitFilters = async (req, res, next) => {
        ...
        try {
          ...
          let data = await Unit.findAll({
            include: [{ as: 'Block', model: Block }, { as: 'UnitType', model: UnitType }, { as: 'PlotSize', model: PlotSize }, { as: 'Member', model: Member }, { as: 'MemNominee', model: MemNominee }, { as: 'Street', model: Street }],
            where: {
                [Op.or]: [filter]
            }
          });
