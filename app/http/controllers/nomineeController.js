...
class NomineeController {
	static addNominee = async (req, res, next) => {
		const { MEMBER_ID, NomineeName, NomineeFatherName, NomineeRealtion, NomineeCNIC, RelationToOwner, TRSR_ID } =
			req.body;

		if (!(MEMBER_ID && NomineeName && NomineeFatherName && NomineeRealtion && NomineeCNIC && RelationToOwner)) {
		...
		try {
			...
			if (TRSR_ID) {
				...
				await TRSRequest.update({ BUYER_MEMBER_NOMINEE_ID: row.MN_ID }, { where: { TRSR_ID: TRSR_ID } });
			}
			res.status(200).json({
				status: 200,
