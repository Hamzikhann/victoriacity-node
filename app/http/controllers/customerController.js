...
class CustomerController {
	static addCustomer = async (req, res, next) => {
		...
		if (fullName && fatherName && dob && cnic && email && designation && companyName && gender) {
	...
	static getCustomerById = async (req, res, next) => {
		...
		let projects = [];
		try {
			const custById = await Customer.findAll({
				include: [{ as: "designationDetail", model: Designation }],
				where: { id: custId }
			});
			if (custById[0]) {
	...
	static getCustomerProject = async (req, res, next) => {
		...
		let projects = [];
		try {
			const custById = await CustomerProject.findAll({
				include: [
					{ as: "project", model: Project },
					{ as: "customer", model: Customer }
				],
				where: { id: custId }
			});
