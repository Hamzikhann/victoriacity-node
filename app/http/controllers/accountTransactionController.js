...
class AccountTransactionController {
	static addAccountTransaction = async (req, res, next) => {
		const { amount, categoryId, date, type, projectId, receiptId, ledgerNo, description, balance } = req.body;
	...
	static receiptsToTransaction = async (req, res, next) => {
		try {
			...
			return res.status(200).json({ status: 200, Message: "Data Fetch Successfully" });
		} catch (error) {
			next(error);
		}
	};
	...
	static updateAccountTransaction = async (req, res, next) => {
		try {
			...
			if (category.length <= 0) {
				return res.send(400).json({ status: 400, Message: "Category Not found" });
			}
			...
			if (projectId == 2 && type == "income") {
				let installmentReceipts = await InstallmentReceipts.findAll({
					attributes: [
						"InsType_ID",
						"INS_RC_ID",
						[Sequelize.fn("SUM", Sequelize.col("Installment_Paid")), "Installment_Paid"],
						"Installment_Month",
						"INSTRUMENT_NO",
						"PMID",
						"IRC_Date",
					...
					group: ["InsType_ID", Sequelize.literal("MONTH(IRC_Date)"), Sequelize.literal("YEAR(IRC_Date)")],
					order: [
	...
	static async uploadCSV(req, res) {
		try {
			...
			const records = XLSX.utils.sheet_to_json(workbook.Sheets[sheet_name_list[1]]);
			let data = [];
			let titles = [];
			let category = null;
				...
				if (title) {
					...
					if (category) {
						if (typeof item["DATE"] == "number") {
							if (item["FOLIO"] == 2262) {
								data.push({
									amount: item["DEBIT"] ? item["DEBIT"] : item[" DEBIT "] ? item[" DEBIT "] : item[" DEBIT  "],
									balance: item["BALANCE"] ? item["BALANCE"] : item[" BALANCE "],
									categoryId: category.id,
									ledgerNo: item["FOLIO"],
									date: new Date(Math.round((item["DATE"] - 25569) * 86400 * 1000)).toISOString().substring(0, 10),
									type: type,
	...
	static partnerData = async (req, res, next) => {
		try {
			...
			for (const project of allProjects) {
				...
				let totalIncome = allIncome.map((item) => item.amount).reduce((acc, curr) => acc + +curr, 0);

				const totalExpense = allExpense.map((item) => item.amount).reduce((acc, curr) => acc + +curr, 0);

				if (projectId == 2) {
					totalIncome = installments.map((item) => item.amount).reduce((acc, curr) => acc + +curr, 0);
				}
			...
			const totalLiabilityAmount = liability.map((item) => item.balance).reduce((acc, curr) => acc + +curr, 0);

			const totalWithdrawalBalance = withdrawal.map((item) => item.balance).reduce((acc, curr) => acc + +curr, 0);
			...
			const totalProjectExpense = projectData
				...
				.reduce((acc, curr) => acc + +curr, 0);
			const totalProjectProfit = projectData.map((project) => project.profit).reduce((acc, curr) => acc + +curr, 0);
			const payoff = await PayOff.findAll();
			const totalPayoff = payoff.map((project) => project.amount).reduce((acc, curr) => acc + +curr, 0);

			const liabilityProfit = totalProjectIncome - totalProjectExpense - totalLiabilityAmount;
			const WithdrawalProfit = totalProjectIncome - totalProjectExpense - totalWithdrawalBalance;
			const totalProfit = totalProjectProfit - totalLiabilityAmount - totalWithdrawalBalance;
			const spotClosing = totalProjectIncome - (totalProjectExpense + +totalPayoff + totalWithdrawalBalance);
			return res.status(200).json({
				status: 200,
				...
				data: [
					{
						...
						totalProjectProfit,
						spotClosing,
						payoff: totalProjectExpense + +payoff.total + totalWithdrawalBalance
					}
				]
			});
		} catch (err) {
	...
	static specificIncomeToTransaction = async (req, res, next) => {
		try {
			...
			installmentReceipts.map(async (receipt) => {
				...
				res.status(200).json({ status: 200, Message: "Data Fetch Successfully" });
		} catch (error) {
			next(error);
		}
	};
	...
	static downloadCsv = async (req, res) => {
		try {
			...
			res.setHeader("Content-Disposition", `attachment; filename=AccountTransactions.csv`);
