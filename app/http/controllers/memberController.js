...
class MemberController {
    static addMember = async (req, res, next) => {
        console.log("hello")
        const { Mem_Reg_Code, BuyerName, BuyerContact, BuyerAddress, BuyerCNIC, FathersName, PermanantAddress, DOB, Email, Relation, Rmarks, IsActive,BuyerSecondContact,TRSR_ID, memberType } = req.body
        console.log(Mem_Reg_Code, BuyerName, BuyerContact, BuyerAddress, BuyerCNIC, FathersName, PermanantAddress, DOB, Email, Relation, Rmarks, IsActive);
        ...
        if (!(Mem_Reg_Code && BuyerName && BuyerContact && BuyerAddress && BuyerCNIC && FathersName && PermanantAddress && DOB && Email && Relation && Rmarks && IsActive)) {
            ...
            if (TRSR_ID) {
                ...
                if (memberType == 1) {
                  await TRSRequest.update(
                    { BUYER_MEMBER_ID: addMember.MEMBER_ID },
                    { where: { TRSR_ID: TRSR_ID } }
                  );
                } 
                if(memberType == 2) {
                  await TRSRequest.update(
                    { BUYER_SECOND_MEMBER_ID: addMember.MEMBER_ID },
                    { where: { TRSR_ID: TRSR_ID } }
                  );
                }
              }
    ...
    static getMemberById = async (req, res, next) => {
        ...
        try {
            const member = await Member.findOne({ include: [{ as:"Member_Adress",model: memberAddress }], where: { MEMBER_ID: MemberId } })
            if (!member) {
    ...
    static getAllMember = async (req, res, next) => {
        ...
        try {
            const { count, rows } = await Member.findAndCountAll({
                offset: limit * page,
                limit: limit,
                order: [["createdAt", "DESC"]], include: [{ as:"Member_Adress",model: memberAddress }]
            });
