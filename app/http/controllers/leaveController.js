...
class leaveController {
    ...
    static getLeaveById = async (req, res, next) => {
        ...
        try {
            const leaveById = await Leave.findAll({ include: [{ as: 'employee', model: Employee }, { as: 'user', model: User }], where: { id: leaveId } })
    ...
    static getLeaveByEmployeeId = async (req, res, next) => {
        ...
        console.log(req.params)
        try {
            const leaveById = await Leave.findAll({ include: [{ as: 'employee', model: Employee }, { as: 'user', model: User }], where: { employeeId: leaveId } })
            if (leaveById.length > 0) {
                res.status(200).json({
                    "status": 200,
                    "message": "get leave successfully",
