    ...
    try {
      ...
      if (!TRSRequestRes || new Date(TRSRequestRes.Expire_Date) < new Date()) {
        ...
        if (jsonData) {
          ...
          let maxInstallment_Code = await InstallmentReceipts.max(
            "Installment_Code"
          );
          ...
          var [row, created] = await InstallmentReceipts.findOrCreate({
            ...
            defaults: {
              ...
              INSTRUMENT_DATE:
                body.instrument_date != "" ? body.instrument_date : null,
  ...
  static processingFee = async (body) => {
    try {
      ...
      const voucherData = await Voucher.create({
        ...
        VOUCHER_DATE: new Date(),
        PAYEE_NAME:
          body.receipt_head !== "transfer_fee" && body.TPC_ID == 2
            ? body.buyer_name
            : member.BuyerName,
        BK_ID: body.BK_ID,
        BK_Reg_Code: BookingO.BK_Reg_Code,
      ...
      if (
        trs &&
        trs.VOUCHER_BUYER_ID &&
        trs.VOUCHER_SELLER_ID &&
        trs.VOUCHER_Transfer_FEE_ID
      ) {
        await TRSRequest.update(
          { status: 1 },
          {
  ...
  static checkConsecutiveUnpaidInstallments = async () => {
    ...
    for (let i = 0; i < distinctBK_IDs.length; i++) {
      ...
      for (const item of installmentReceipts) {
        const IR = installmentReceipt.find(
          (el) => el.BKI_DETAIL_ID === item.BKI_DETAIL_ID
        );
  ...
  static checkConsecutiveTenMonthUnpaidInstallments = async () => {
    ...
    try {
      ...
      for (let i = 0; i < distinctBK_IDs.length; i++) {
        ...
        const installmentReceipts = installmentReceiptsAll.filter(
          (it) => it.BK_ID == BK_ID && it.BKI_TYPE != "DC"
        );
        let installmentReceipt = installmentReceiptAll.filter(
          (it) => it.BK_ID == BK_ID
        );
        ...
        let ostMonths = await this.getOutStandingMonths(
          installmentReceipts,
          installmentReceipt
        );
