...
class pdfGenerator {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        dataArr.push([
          ...
          {
            text: `${
              rows[i]?.PlotSize?.Size_Marla
                ? rows[i]?.PlotSize?.Size_Marla + " Marla"
                : ""
            }`,
            fontSize: 8,
            border: [true, true, true, true],
                ...
                text: [
                  {
                    text:
                      "Date : " +
                      `${
                        body?.FileSubmission?.Date
                          ? formatTimestamp(body?.FileSubmission?.Date, 0)
                          : ""
                      }`,
                    bold: true,
                  },
                ...
                text: [
                  {
                    text:
                      "Delivery Time : " +
                      `${
                        body?.FileSubmission?.DD_Time
                          ? FileService.convertToAMPM(
                              body?.FileSubmission?.DD_Time
                            )
                          : ""
                      }`,
      ...
      const filePath =
        "uploads/fileReceipt/" +
        `FileSubReceipt-FSRC-${body.FileSubmission.FSRC_Code}` +
        ".pdf";
    ...
    try {
      ...
      var docDefinition = {
        ...
        content: [
              ...
              },
              {
                ...
                stack: [
                  ...
                  },
                  {
                    text: [
                      {
                        text: body?.SecondMember?.BuyerCNIC ? [{text: 'CNIC : ', bold: true }, `${body?.SecondMember?.BuyerCNIC}`]: "" ,
          ...
          {
            text:
              "Subject: Acknowledgment Letter for Booking Application (" +
      ...
      const filePath =
        "uploads/acknowlegmentLetters/" +
        `VC-Acknowlegment-Letter-` +
        body.BK_ID +
        ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
  ...
  static paymentPlanGenerator = async (bookingData, insRecpData,installmentReceipt, receipt_head,TRSData,devInsReceipts) => {
...
if (receipt_head) {
  ...
  lastHeading="Total Installment Amount ";
    ...
    insRecpData.map(async (item, i) => {
      ...
      const insType = installmentTypes.find(
        (el) => el.InsType_ID === item.InsType_ID
      );
      if(receipt_head){
    ...
    try {
        ...
        content: [
          ...
          },
          {
            table: {
              ...
              widths: !bookingData.PaymentPlan.IncludeDC ?["25%", "25%", "30%", "25%", "25%"]:  ["8%", "25%", "25%", "20%", "20%"],
              body: arr2,
            },
      ...
      const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
      const filePath =
        "uploads/paymentPlan/VC-Payment-Plan" + bookingData.BK_ID + ".pdf";
  ...
  static statementGenerator = async (
    bookingData,
    insRecpData,
    dcInsRecpData,
    ...
    insRecpData.map(async (item, i) => {
      ...
      const instDay = parseInt(
        item.Due_Date ? item.Due_Date.split("-")[2] : ""
      );
      ...
      if (IROBJECTS.length == 0) {
        ...
        for (var k = 0; k < IROBJECTS.length; k++) {
          const paymentMode = PaymentModes.find(
            (el) => el.PMID == IROBJECTS[k]?.PMID
          );
          var insTypeO = installmentTypes.find(
            (el) => el.InsType_ID === IROBJECTS[k].InsType_ID
          );

          const FormatedStampDateO = formatTimestampf(IROBJECTS[k].IRC_Date, 0);
          const FormatedMonthO = formatTimestamp(
            IROBJECTS[k].Installment_Month,
            1
          );
          if (PaidR) {
    ...
    try {
      ...
      const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
      const filePath =
        "uploads/statement/VC-Statement-" + bookingData.BK_ID + ".pdf";
  ...
  static statementGeneratorZip = async (
    bookingData,
    insRecpData,
    installmentPaidReceipts
  ) => {
    ...
    array4 = [
      ...
      {
        text: `${
          remainingOst - paidAmount > 0 ? remainingOst - paidAmount : 0
        }`,
        alignment: "right",
        bold: true,
        fontSize: 9,
  ...
  static getUnVarifiedReceiptsPdf = async (installment) => {
    try {
      ...
      const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
      const filePath =
        "uploads/unVarifiedTransactions/VC-unVarifiedTransactions.pdf";
  ...
  static getUnVarifiedPdfByUser = async (data, grandTotalAmount) => {
    ...
    try {
        ...
        if (data.hasOwnProperty(key)) {
          ...
          data[key].items.forEach(async (item) => {
            ...
            const unitType = UnitTypes.find(
              (el) => el.UType_ID == item["Booking.UType_ID"]
            );
            const plotSize = PlotSizes.find(
              (el) => el.PS_ID == item["Booking.PS_ID"]
            );
            // const BookingObj = await Booking.findAll({ where: { BK_Reg_Code: item?.BK_Reg_Code } });
      ...
      const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
      const filePath =
        "uploads/unVarifiedTransactions/VC-unVarifiedTransactionsPaymentWise.pdf";
  ...
  static cashReceiptGenerator = async (body, rows, trsrequest) => {
    ...
    if (body && body?.Booking_Installment_Details?.Due_Date) {
      FormatedDueDate = formatTimestampf(
        body?.Booking_Installment_Details?.Due_Date
      );
    }
    let FormatedInstallmentDate = "";
    if (body && body?.Booking_Installment_Details?.Installment_Month) {
      FormatedInstallmentDate = formatTimestampf(
        body?.Booking_Installment_Details?.Installment_Month,
        1
      );
    }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        let dataNeArr = [
          ...
          {
            text: `${rows[i]?.Voucher?.TaxPayeeCategory?.Name == 'Seller'? `${trsrequest?.VoucherSellerTaxId?.PAYEE_NAME || '' } (${rows[i]?.Voucher?.TaxPayeeCategory?.Name}) \n ${trsrequest?.VoucherSellerTaxId?.Cnic}` : `${trsrequest?.VoucherBuyerTaxId.PAYEE_NAME || ' '} (${rows[i]?.Voucher?.TaxPayeeCategory?.Name}) \n ${trsrequest?.VoucherBuyerTaxId?.Cnic}` || " "}`,
            fontSize: 10,
            border: [true, true, true, true],
            borderColor: " #91CBFF",
            margin: 2,
            ...
            columns: [
                      ...
                      `${
                        body?.Installment_Month
                          ? body?.Installment_Month?.split("-")[1] == 10
                            ? body?.Installment_Month
                              ? fullMonthsArr[
                                  body?.Installment_Month?.split("-")[1] + ""
                                ]
                              : ""
                            : fullMonthsArr[
                                (
              ...
              // Customer Copy
              {
                ...
                text: `${
                  body?.Installment_Month
                    ? "Installment Year : " +
                      body?.Installment_Month?.split("-")[0]
                    : ""
                }`,
                alignment: "left",
                // decoration: "underline",
          ...
          // Date and Deliver Date Section
          {
            columns: [
              ...
              // Deliver Date
              {
                ...
                text: [
                  {
                    text: `${(typeof FormatedDueDate != "undefined" && FormatedDueDate)? "Due Date : " + `${FormatedDueDate}`: ""}` ,
                    bold: true,
                  },
                ],
                alignment: "left",
            ...
            columns: [
              // Mobile No
              {
                ...
                text: [
                  {
                    text:
                      "Payment Mode : " + `${body?.Payment_Mode?.Description}`,
                    bold: true,
                  },
                ],
                margin: [5, 0, 0, 0],
      ...
      const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
      const filePath =
        "uploads/voucherRCReport/" +
        `VoucherRCReport-${body.INS_RC_ID}` +
        ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
      return filePath;
  ...
  static NDC_Report_Generator = async (body, rows, outstandingAmt, user) => {
    try {
      ...
      let username = fullName.replace(/\s/g, "").toLowerCase();
      const filePath =
        "uploads/ndcfee/" + `NDCFee-Report-${body.INS_RC_ID}` + ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
        ...
        content: [
          {
            ...
            columns: [
              ...
              [
                ...
                {
                  text: `Printing Date : ${moment(new Date()).format(
                    "DD-MMM-YYYY"
                  )}`,
                  fontSize: 8,
                  bold: true,
  ...
  static ballotAllotLetter = async (body, index, data) => {
    try {
            ...
            columns: [
              {
                ...
                stack: [
                  {
                    text: `Reference # VCPAL02${this.padWithZeros(
                      index + 1,
                      4
                    )}`,
                    alignment: "left",
      ...
      const filePath =
        "uploads/ballotAllotLetter/" + body.Reg_Code_Disply + ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
      // console.log('QQQQQQQQQQQQQQQQQQQQ', pdfDoc)
      return filePath;
  ...
  static developmentChargesLetter = async (body, index, data) => {
    try {
            ...
            columns: [
              {
                ...
                stack: [
                  ...
                  },
                  {
                    text: [
                      { text: "Category : " },
                      { text: `${body?.UnitType?.Name ?? 'NIL'}`, bold: true },
                    ],
                    alignment: "left",
                    bold: true,
                    fontSize: 9,
              ...
              },
              {
                ...
                stack: [
                  ...
                  },
                  {
                    text: [
                      {
                        text: `${body?.SecondMember?.FathersName ? `S/O, D/O, W/O  ${body?.SecondMember?.FathersName}` : ""}`,
                        fontSize: 9,
                      },
                    ],
                    bold: true,
                  ...
                  },
                  {
                    text: [
                      {
                        text: `${body?.SecondMember?.BuyerContact ? `Phone No  +${body?.SecondMember?.BuyerContact.replace(/-/g, '')}` : ""}`,
                        fontSize: 9,
                      },
                    ],
                    bold: true,
          ...
          columns: [
            {
              ...
              stack: [
                ...
                {
                  text: `Printed on ${moment(new Date()).format("Do MMMM YYYY")}. Please note this document is not a proof of ownership. Error and omissions can be expected.`,
                  alignment: "left",
                  fontSize: 9,
                  bold: true,
                  margin: [75, 70, 70, 0],
      ...
      const filePath =
        "uploads/developmentChargesLetter/" + body?.PlotSize.Size_Marla + '-' + body?.UnitType.Name + '-' +  body.Reg_Code_Disply + ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
      // console.log('QQQQQQQQQQQQQQQQQQQQ', pdfDoc)
      return filePath;
  ...
  static transferLetter = async (body, index, data, user, total, paid, TRSData) => {
    try {
                ...
                stack: [
                  {
                    text: [
                      {
                        text: `${TRSData?.secondMember?.BuyerName ? `Member Name: ${TRSData?.secondMember?.BuyerName}`: '' }`,
                        fontSize: 9,
                        bold: true,
                      },
                    ],
                 ...
                 
                  {
                    text: [
                      {
                        text: `${TRSData?.secondMember?.BuyerContact ? `Contact: ${TRSData?.secondMember?.BuyerContact}` : ''}`,
                        fontSize: 9,
                        bold: true,
                      },
                    ],
      ...
      const filePath =
        "uploads/transferLetter/" + body.Reg_Code_Disply + ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
      // console.log('QQQQQQQQQQQQQQQQQQQQ', pdfDoc)
      return filePath;
  ...
  static transferEventCapture = async (body, rows,TRSData, user) => {
    try {
      ...
      let username = fullName.replace(/\s/g, "").toLowerCase();
      const filePath =
        "uploads/transferEvent/" + `transfer-Event-${body.id}` + ".pdf";
      pdfDoc.pipe(fs.createWriteStream(filePath));
      pdfDoc.end();
      return filePath;
    } catch (error) {
