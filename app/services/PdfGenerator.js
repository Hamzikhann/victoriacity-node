...
class pdfGenerator {
				...
				content: [
...

					{
						...
						text: [
							...
							{
								text: "Acknowledgment Letter for Booking Application (" + body.Reg_Code_Disply + ")",
								fontSize: 12,
								decoration: "underline"
							}
						],
					...
					{
						text: "Aslaam 0 Alaekum !",
						margin: [0, 5, 0, 0],
						fontSize: 12
					},
					{
						text: `The Management is pleased to inform you that the subject application has been received subject to the terms and conditions as mentioned on the booking Application and those which may be enforced in future by the Management of Victoria Estates Private Limited or any authority/entity competent to do so.`,
						margin: [0, 23, 0, 0],
						fontSize: 11,
						lineHeight: 1.5,
						//   bold:true,
					...
					{
						text: `Please keep this acknowledgement letter and the certified copy of the Booking Application Form in safe custody for future correspondence.`,
						margin: [0, 23, 0, 0],
						fontSize: 11,
						lineHeight: 1.5,
						leadingIndent: 40
					...
					{
						text: "Assuring you our the best services and co-operation.",
						margin: [0, 23, 0, 0],
						fontSize: 12,
						// bold: true
						leadingIndent: 40
					...
					{
						text: "(Lt Col Anwer Mahmood, Retd)",
						margin: [0, 10, 0, 0],
						fontSize: 11,
						bold: true,
						alignment: "right"
					...
					{
						text:
							`Note:	For any query / discrepancy, please contact this office within 15 days from the date of printing.
	...
	static SurchargeGenerator = async (body, rows, receiptHead) => {
		const fonts = {
			Roboto: {
				normal: path.resolve("./resources/fonts/roboto/Roboto-Regular.ttf"),
				bold: path.resolve("./resources/fonts/roboto/Roboto-Medium.ttf"),
		...
		try {
			...
			const arrayNewTableData = [
				{
					text: `${body?.member ? body?.member : "NIL"}`,
					alignment: "left",
					fontSize: 9,
					border: [true, true, true, true],
					borderColor: " #91CBFF"
				...
				content: [
					{
						...
						columns: [
							...
							// 2nd Heading with below sub-heading///
							{
								...
								stack: [
									{
										text: `Receipt No: VCA00${body?.BK_ID}`,
										bold: true,
										fontSize: 10,
										margin: [0, 5, 50, 0]
									},
									{
										text: `Generated By: Cashier`,
										bold: true,
										fontSize: 10,
										margin: [0, 5, 50, 0]
									}
			...
			// create invoice and save it to invoices_pdf folder
			const pdfDoc = printer.createPdfKitDocument(docDefinition, options);
			const filePath = "uploads/surChargeRCReport/" + `surChargeRCReport-${body.BK_ID}` + ".pdf";
			pdfDoc.pipe(fs.createWriteStream(filePath));
			pdfDoc.end();
			return filePath;
		} catch (error) {
	...
	static RestorationFeeGenerator = async (body, rows, receiptHead) => {
		const fonts = {
			Roboto: {
				normal: path.resolve("./resources/fonts/roboto/Roboto-Regular.ttf"),
				bold: path.resolve("./resources/fonts/roboto/Roboto-Medium.ttf"),
	...
	static testing = async (bookings) => {
		...
		// 		filePath = "uploads/newfile/" + `noticeLetter${i}` + ".pdf";
		// 		pdfDoc.pipe(fs.createWriteStream(filePath));
		// 		pdfDoc.end();
		// 	}
		// 	// )
		...
		try {
			// Process bookings in batches to avoid memory overload
			const batchSize = 10;
			await pdfGenerator.processBookingsInBatches(bookings, batchSize);
		} catch (error) {
			console.log("ERRRRRRRRRRRRRRRRRRRRR", error);
		}
	};
	...
	static async processBatch(batch, startIndex) {
		const fonts = {
			NotoNastaliqUrdu: {
				normal: path.resolve("./resources/fonts/noto/NotoNastaliqUrdu-Regular.ttf"),
				bold: path.resolve("./resources/fonts/noto/NotoNastaliqUrdu-Bold.ttf")
				// italics: path.resolve("./resources/fonts/roboto/Roboto-Italic.ttf"),
				// bolditalics: path.resolve("./resources/fonts/roboto/Roboto-MediumItalic.ttf")
			}
		};
		...
		for (let j = 0; j < batch.length; j++) {
			const booking = batch[j];
			const index = startIndex + j;

			console.log(booking.BK_ID);
			// Format address logic
			let address = booking.Member?.BuyerAddress ?? booking.Member?.PermanantAddress ?? null;
			let formattedAddress = address ? pdfGenerator.splitAddress(address, 40) : null;
			...
			var docDefinition = {
				...
				content: [
					...
					},
					{
						...
						stack: [
							{
								text: `Name : ${booking.Member?.BuyerName ? booking.Member.BuyerName : null}`,
								fontSize: 10,
								margin: [0, 0, 0, 0],
								bold: true
							},
							{
								text: `Phone No: ${booking.Member?.BuyerContact ? booking.Member.BuyerContact : null}`,
								alignment: "left",
								fontSize: 10,
								bold: true,
								margin: [0, 0, 0, 0]
				...
				defaultStyle: {
					font: "NotoNastaliqUrdu" // Set the default font to your custom font
				}
			};
			...
			// Save the PDF file
			var filePath = `uploads/newFiles/noticeLetter${booking.Reg_Code_Disply}.pdf`;
