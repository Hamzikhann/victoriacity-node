...
const JobCandidateStatus = require("../models/JobCandidateStatus.js");
const CustomErrorHandler = require("../services/CustomErrorHandler.js");
const mailer = require("nodemailer");
const dotenv = require("dotenv");
// const { WhatsAppAPI,Types} = require('whatsapp-api-js');
...
class CandidateStatus {
	static updateCandidateStatus = async (body, res, next) => {
		if (
			body.hasOwnProperty("candId") &&
			body.hasOwnProperty("status") &&
			body.hasOwnProperty("interviewDate") &&
			body.hasOwnProperty("interviewTime") &&
			body.hasOwnProperty("interviewDesc")
		) {
			const { candId, status, interviewDate, interviewTime, interviewDesc } = body;
			try {
				const candObj = await JobCandidate.update(
					{
						status: status,
				...
				const candObject = await JobCandidate.findOne({
					where: { id: candId }
				});
			...
			try {
				const candWithRemarks = await JobCandidate.update(
			...
			try {
				const candUpdateDateOfJoining = await JobCandidate.update(
				...
				if (candUpdateDateOfJoining[0] === 1 && candDateOfJoining.status === "Send Offer Letter") {
					...
					const mailOptions = {
						...
						html:
							"<div><h4>Dear Mr. " +
							candDateOfJoining.firstName +
							" " +
							candDateOfJoining.lastName +
							'</h4><p><span style="padding-left:100px; ">We</span> are excited to offer you a full-time position as a <b>Graphic Designer</b> at <b>Sheranwala Developers</b>. Based on your experience, interviews and design portfolio, we look forward to seeing how you will take our brand messaging to the next level.<br><br><span style="padding-left:100px; ">If</span> you decide to accept this role, your anticipated start date will be <b>' +
							candDateOfJoining.dateOfJoining +
							"</b> at our 1234 Southern Avenue location. You will be expected to work 40 hours per week, Monday through Friday with the option to work remotely up to two days per week. Please find attached an updated copy of the job description to familiarize yourself with some of the position’s duties and responsibilities.<br><br>Sincerly,<br>HR Manager</p></div>"
					};
